...
import ExamAgent, { AgentError, AgentErrorType } from './aiAgent';
import type { Question, UserProfile } from "../types";
import type { EnrollmentData, ScoreLine } from "../data";

// 初始化智能体
let agent: ExamAgent | null = null;

/**
 * 初始化智能体实例
 */
function initAgent(): ExamAgent {
  if (!agent) {
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    if (!apiKey) {
      console.warn('VITE_GEMINI_API_KEY not found, using mock mode');
      throw new Error('API key not configured');
    }
    agent = new ExamAgent(apiKey);
  }
  return agent;
}
...
export async function generateQuestion(
  ...
  try {
    const agentInstance = initAgent();
    return await agentInstance.generateQuestion(subject, knowledgePoint, type);
  } catch (error) {
    console.error('Question generation failed, using fallback', error);
    return generateQuestionFallback(subject, knowledgePoint, type);
  }
}
...
export async function evaluateAnswer(
  ...
  try {
    const agentInstance = initAgent();
    return await agentInstance.evaluateAnswer(question, userAnswer);
  } catch (error) {
    console.error('Answer evaluation failed, using fallback', error);
    return evaluateAnswerFallback(question, userAnswer);
  }
}
...
export async function generateSimilarQuestions(
  ...
  try {
    const agentInstance = initAgent();
    return await agentInstance.generateSimilarQuestions(question, count);
  } catch (error) {
    console.error('Similar questions generation failed, using fallback', error);
    return generateSimilarQuestionsFallback(question, count);
  }
}
...
export async function chatWithTutor(
  ...
  try {
    const agentInstance = initAgent();
    return await agentInstance.chat(message, {
      userProfile: userProfile || {},
    ...
    return chatWithTutorFallback(history, message, userProfile, enrollmentData, scoreData);
  }
}
 ...
 * 题目生成降级
 */
function generateQuestionFallback(
  subject: string,
  knowledgePoint: string,
  type: string
): Promise<Question> {
  return new Promise(async (resolve) => {
      ...
      '微观经济学': [
        {
          ...
          answer: '需求增加',
          analysis:
            '替代品是指功能相同或相似，可以满足消费者同一需要的商品。当一种商品价格上升，消费者会减少对该商品的需求，转而增加对其替代品的需求。',
          knowledgePoint: '需求理论',
          subject: '微观经济学',
        },
        {
          ...
          answer: 'P=MC',
          analysis:
            '在完全竞争市场，厂商是价格的接受者，P=MR。利润最大化条件MR=MC即演变为P=MC。',
          knowledgePoint: '市场结构',
          subject: '微观经济学',
        },
      ],
      '宏观经济学': [
        {
          ...
          answer: 'IS曲线右移',
          analysis:
            '政府支出是IS曲线的移动因素。政府支出增加属于扩张性财政政策，会使IS曲线向右上方移动。',
    ...
    const subjectQuestions =
      mockQuestions[subject] || mockQuestions['微观经济学'];
    const randomQ =
 ...
 * 答案评估降级
 */
function evaluateAnswerFallback(
  question: Question,
  userAnswer: string
): Promise<{
  isCorrect: boolean;
  ...
  return new Promise(async (resolve) => {
    ...
    resolve({
      ...
      recommendedCourse: isCorrect
        ? undefined
        : `${question.subject}精讲 - ${question.knowledgePoint}`,
    });
  });
}
 ...
 * 相似题生成降级
 */
function generateSimilarQuestionsFallback(
  question: Question,
  count: number = 5
): Promise<Question[]> {
  return new Promise(async (resolve) => {
 ...
 * AI对话降级
 */
function chatWithTutorFallback(
  history: { role: string; text: string }[],
  message: string,
  userProfile?: UserProfile,
  enrollmentData?: EnrollmentData[],
  ...
  return new Promise(async (resolve) => {
    ...
    if (isVolunteering || isAdjustment) {
      ...
      if (userProfile?.university && userProfile?.major) {
        const targetEnroll = enrollmentData?.find(
          (e) =>
        ...
        const targetScore = scoreData?.find(
          (s) =>
    ...
    if (message.includes('分数线') || message.includes('科目')) {
      resolve(
        '关于你询问的目标院校信息：通常经济学学硕考察政治、英语一、数学三和专业课（如801经济学）；专硕则可能考察英语二和431金融学综合。具体分数线建议参考该校研招网近三年的复试线，一般名校在360-400分之间。你需要我针对某个具体学校进行分析吗？'
      );
      return;
    }

    resolve(
      '你好！我是你的经济学考研助手。目前AI服务暂时不可用，正在使用降级模式。我可以为你解答专业课难点、分析院校选择策略或提供复试调剂建议。请问今天有什么我可以帮你的？'
    );
  });
}
...
export const aiModel = "gemini-1.5-pro";
