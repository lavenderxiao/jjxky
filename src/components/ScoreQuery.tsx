...
import { Search, Send, Table, Users, MessageSquare, ArrowLeft, Loader2, Maximize2, Minimize2, X } from 'lucide-react';
...
type ExpandedView = 'none' | 'scores' | 'enrollment';
...
export default function ScoreQuery({ userProfile, onBack }: ScoreQueryProps) {
  ...
  // 分数线表格组件
  const ScoreTable = ({ isExpanded = false }: { isExpanded?: boolean }) => (
    <>
      <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => setExpandedView(isExpanded ? 'none' : 'scores')}
            className="p-2 hover:bg-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 transition-all"
  ...
  // 招生人数表格组件
  const EnrollmentTable = ({ isExpanded = false }: { isExpanded?: boolean }) => (
    <>
      <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => setExpandedView(isExpanded ? 'none' : 'enrollment')}
            className="p-2 hover:bg-slate-200 rounded-lg text-slate-500 hover:text-emerald-600 transition-all"
            title={isExpanded ? '缩小' : '扩大'}
          >
            {isExpanded ? <Minimize2 size={18} /> : <Maximize2 size={18} />}
          </button>
      ...
      <AnimatePresence>
        {expandedView !== 'none' && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 p-6"
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="w-full h-full bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden"
            >
              {expandedView === 'scores' ? (
                <ScoreTable isExpanded />
              ) : (
                <EnrollmentTable isExpanded />
              )}
            </motion.div>
          </motion.div>
        )}
